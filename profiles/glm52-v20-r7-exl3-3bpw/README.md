...
| Auto-sized GPU KV capacity | 694,528 tokens |
| Full-context concurrency | 2.65x |
| Maximum batched tokens | 3,072 |
| Maximum sequences | 8 |
| GPU memory utilization | 0.96 |
| KV type | `nvfp4_ds_mla` |
| KV record size | 432 bytes |
| FP8 RoPE | Disabled |
| Persistent CKV workspace | 486.5 MiB/GPU |
...
optimized full-CKV gather path. Raising it consumes approximately 486.5 MiB
per GPU but keeps the optimized path eligible throughout the supported
context window. Auto KV still provides 694,528 tokens, or 2.65 full-length
requests.
...
```text
Preallocated 486.5 MiB for 2 persistent CKV execution lane(s)
GPU KV cache size: 694,528 tokens
Maximum concurrency for 262,144 tokens per request: 2.65x
