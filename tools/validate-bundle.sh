...
if rg -n -i '(password|access[_-]?token|auth[_-]?token|hf[_-]?token|api[_-]?key|private[_-]?key|/home/[^/]+|/root/|192\.168\.|10\.[0-9]+\.|172\.(1[6-9]|2[0-9]|3[01])\.)' "$path" --glob '!*.md' --glob '!*.example'; then
